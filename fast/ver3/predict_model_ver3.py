...
# -*- coding: utf-8 -*-
"""
사망자 수 예측 모델 - Java 연동 최적화 버전 (지역 코드 로직 제거)
- UTF-8 인코딩 처리
- 지역별 사망자 비율 계산
- 2024-2025년 통합 예측
"""
...
def create_metadata(num_regions):
    """메타데이터 생성 (속성 설명)"""
    metadata = {
...
def save_predictions_to_json(predictions, output_file):
    ...
    # 지역 수 계산
    num_regions = len(set(p['regionName'] for p in predictions))
    
    # 메타데이터 생성
    metadata = create_metadata(num_regions)
    ...
    output_data = {
        ...
        "summary": {
            ...
            "years": sorted(list(set([p['year'] for p in predictions]))),
            "regions": sorted(list(set([p['regionName'] for p in predictions])))
        }
    }
...
if __name__ == "__main__":
    ...
    # 파일 경로 설정
    # 사용자 환경에 맞게 이 경로를 수정하세요.
    training_data_path = '/Users/andohyung/Downloads/예측모델/AI_Model/지역_월별_사망자수_데이터_0801.csv' # 예시 파일명
    ...
    if os.path.exists(training_data_path):
        try:
            combined_predictions = main_sequential_prediction(
                ...
                output_combined='predict_result_ver3.json',
                target_months=None  # None이면 1-12월 전체
            )
            
            print("\n✅ 예측이 성공적으로 완료되었습니다!")
            print("결과 파일: predict_result_ver3.json")
